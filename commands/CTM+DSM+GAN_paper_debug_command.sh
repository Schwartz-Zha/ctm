#!/bin/bash
MODEL_FLAGS="--use_fp16=True --use_d_fp16=False --num_heun_step=20 --start_scales=40 --data_name=imagenet64 --image_size=64 --microbatch=16 --global_batch_size=256 --lr=0.000008 --class_cond=True --eval_interval=1000 --save_interval=1000 --num_classes=1000 --eval_batch=250 --eval_fid=True --eval_similarity=False --check_dm_performance=False --log_interval=1"
...
mpiexec -n 1 --hostfile ./commands/hostfile --allow-run-as-root python ./code/cm_train.py $MODEL_FLAGS $CKPT_FLAGS $RESUME_FLAGS $GAN_FLAGS